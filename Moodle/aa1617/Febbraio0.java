...
public class Febbraio0 {
	 ...
	 */
	 public static boolean ex1 (int[] a, int[] b){
		boolean ok = a != null && b != null && a.length != 0 && b.length !=0;
		int check =0;
		int i =0;
		while(ok && i < a.length){
			int count =0;
			int j=0;
			while(!(count==b.length-1)&& j<b.length){
				if(a[i]>b[j])
					count = count +1;
				j = j +1;
			}
	 ...
	 * DOVE SCRIVERE ex2: immediatamente al di sotto di questo commento.  
	 ...
	 public static boolean ex2(int[] a, int k){
		 ...
		 int b; 
		 if(a != null){
		 	b = occorreInPosPari(a, 0, a.length) ;
		 }
		 int i =0;
		 while (b != null && a.length/2 == b.length || i <b.length){
			 if(b[i] == k)
			 check = true;
			i++;
		 }
	 ...
	 private static int occorreInPosPari(int[] a, int l, int r){
		 if(l < r)
		 	if(l+1 == r){
				 if(l%2==0 || l==2){
				 	return a[l];
				...
				return 0;
			 	...
			 	return occorreInPosPari(a, l, (l+r)/2) + occorreInPosPari(a, (l+r)/2, r);	
				else
				return 0; 
